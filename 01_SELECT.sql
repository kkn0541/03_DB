 ...
 * 
 * -RESULT SET 은 0개 이상의 행을 포함할수 있다. 
 * 왜 0개? 조건에 맘ㅈ는 행이 없을 수도 있어서 
 * 
 * */
...
SELECT * FROM EMPLOYEE;  -- 조회에서 나온 값 : RESULT SET 
-- '*' : ALL , 모든 , 전부
--> EMPLOYEE 테이블의 모든 컬럼을 조회하겠다. 


-- EMPLYEE 테이블에서 사번 , 직원이름 , 휴대전화번호만 조회 
...
--EMP테이블에서 이름 읿사일 오늘 잘짜 조회

SELECT EMP_NAME,HIRE_DATE,SYSDATE FROM EMPLOYEE;

--2024-10-08 11:09:12.000
--SYSDATE : 시스템상의 현재 시간(날짜)를 나타내느 상수
 ...
 * 
 * 컬럼명 AS "별칭" : "" 안에서는 별칭 띄어쓰기 O 특수문자 O 문자만 O 
 * 
 * AS 생략가능
 * */
...
--주의사항 1) DISTINCT 구문은 SELECT(마다 )딱 한번씩만 작성ㄱ 가능 
...
--                                        널을 맨 처음 보여줄꺼냐 마지막에 보여줄꺼냐
--기본은 ASC(오름 차순)


--EMPLOYEE 테이블에서 
--급여가 3백마원 초과인 사원의 (WHERE)
...
--EMPLYEE 테이블에서 (FROM 절)
--부서코그다 D9인 사람의 (WHERE 절)
--사번 이름 부서코드 직급코드를 조회 
...
--EMPLYEE 테이블에서 입사일이 1990-01-01 ~1999-12-31 사이인 직원의 이름과 입사일
...
--EMPLOYEE 테이블에서 성이 '전'씨 인 사원의 사번 , 이름 조회 
...
--EMPLOYEE 테이블에서 
--이메일 '_' 앞이 4글자 이면서 (AND)
-- 부서코드가 'D9' 또는 'D6'이고  --> AND 연산자가 OR보다 우선순위가 높다 () 사용가능  
-- 입사일이 1990-01-01~2000-12-31 이고 
--급여가 270만원 이상인 사원의 
-- 사번, 이름, 이메일 , 부서코드 , 입사일 , 급여조회 

SELECT 
EMP_ID,EMP_NAME,EMAIL,DEPT_CODE,HIRE_DATE,SALARY
FROM EMPLOYEE
...
--EMPLOYEE 테이블에서 
--부서코드가 D1, D6 D9 인 사원의 사번 ,이름 , 부서코드 조회 
...
OR DEPT_CODE IS NULL; --부서코드가 없는 2명 포함 총 14명

----------------------------------------------------
/*
 * NULL 처리 연산자 
 * 
 * JAVA에서 NULL : 참조하는 개체가 없음을 의미 
 * DB에서 NULL :컬럼에 값이 없음을 의미하는 값 
 * 
 * 1) IS NULL : NULL 인 경우 주회 
...
ORDER BY 3  DESC; -- 컬럼 순서 SELECT 문의 3번째 SALARY 
...
FROM EMPLOYEE
ORDER BY DEPT_CODE,SALARY DESC;  --DEPT_CODE 먼저 정렬 후 소분류 SALARY(내림차순 정렬) 
