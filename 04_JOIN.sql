...
--DEPARTMENT 테이블의 DET_ID를 연결고리 지정 
...
--DEPARTEMENT 테이블, LOCATION 테이블을 참조하여 
--부서명, 지역명 조회 
SELECT * FROM DEPARTMENT ;
...
--두 테이블의 지정하는 컬럼값이 않는 행도 조인에 포함 시킴 
--> * 반드시 OUTER JOIN 명시해야한다


--1) LEFT OUTER JOIN  합치기에 사용한 두 테이블 중 왼편에 기술된 테이블의
-- 컬럼수를 기준으로 JOIN 
--> 외편에 작성된 테이블의 모든 행이 결과에 포함되어야 한다. 
--(JOIN이 안되는 행도 결과 포함 )

--ANSI 표준 
SELECT EMP_NAME,DEPT_TITLE 
FROM EMPLOYEE LEFT /*OUTER*/ JOIN DEPARTMENT  --왼쪽에 잇는 EMPLOYEE 기준으로 조인
ON(DEPT_CODE=DEPT_ID); --23행 DEPT_CODE가 NULL인 하동운 이오리 도 포함되어서 23행 
...
--ANSI
SELECT EMP_NAME,DEPT_TITLE 
FROM EMPLOYEE RIGHT /*OUTER*/ JOIN DEPARTMENT  --오른쪽 잇는 DEPARTMENT 기준으로 조인
ON(DEPT_CODE=DEPT_ID); --마케팅부 국ㄴ재영업부 해외영업3부는 사람이 없다.  
...
-->JOIN 구문 잘못 작성 하는 경우 CROSS JOIN의 결과가 조회됨 
...
--테이블마다 별칭 작성(미작성시 열의 정의가 애매하다고 오류발생)
...
--EMPLOYEE DEPT_CODE
--DEPARTMENT DEPT_ID
-->잘못 조인하면 CROSS JOIN 결과 조회 (몇백개나옴)
...
SELECT  * FROM EMPLOYEE;  --LOCATION에 대한 정보가 없어서 DEPARTMETN랑 먼저 조인해서 접근
SELECT * FROM DEPARTMENT;
SELECT * FROM LOCATION;
SELECT * FROM JOB;
...
--[다중조인 연습문제]
--직급이 대리이면서 아시자지역에 근무하는 직원조회
--사번 이름 직급명 부서명 근무지역명 급여

SELECT EMP_ID, EMP_NAME, JOB_NAME,  DEPT_TITLE, JOB_NAME, LOCAL_NAME,SALARY 
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE= DEPT_ID)
JOIN LOCATION  ON(LOCATION_ID=LOCAL_CODE)
...
SELECT EMP_NAME , DEPT_CODE,JOB_NAME,DEPT_TITLE
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE= DEPT_ID)
...
SELECT EMP_NAME ,DEPT_TITLE,JOB_NAME ,LOCAL_NAME,DEPT_CODE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE)
JOIN LOCATION ON(LOCATION_ID=LOCAL_CODE)
JOIN JOB USING(JOB_CODE)
