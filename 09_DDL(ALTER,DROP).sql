...
--*** DEPT_COPY 의 DEPT_TITLE 컬럼에 NOT NULL 제약 조건 추가 /삭제 ***
ALTER TABLE DEPT_COPY
...
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NOT NULL; -- DEPT_TITLE 컬럼에 NULL을 비허용 

ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NULL; --DEPT_TITLE 컬럼에 NULL값 허용 
...
--SQL Error [12899] [72000]: ORA-12899: "KH_KKN"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--CHAR(2) -> 고정길이로 2BYTE 까지만 저장 가능 
--> DEPT_ID의 데이터타입이 CHAR(2) 이므로 영어 +숫자 2글자까지 저장가능
--> D10 은 3바이트 CHAR(3)에 들어올수 있음 

-->VARCHAR2(3) 으로 변경해 보기 (남은 바이트 메모리 반환을 위해)
...
--DEPARTMENT 테이블복사해서 DEPT_COPY 생성
CREATE TABLE DEPT_COPY 
AS SELECT * FROM DEPARTMENT;
...
--DROP TABLE 테이블명 [CASCADE CONTRAINTS];
...
CREATE TABLE TB2(
	...
	TB2_COL NUMBER REFERENCES TB1   -- TB1 의 PRIMARY KEY 참조  
); --자식테이블
...
-->해결방법 
-- 1) 자식 , 부모테이블 , 순서로 삭제 
--2) ALTER 를 이용해러 FK 제약조건 삭제 후 TB1 삭제 
--3) DROP TABLE 삭제 옵션 CASCADE CONSTRAINT 사용 
	-->CASCADE CONSTRAINT : 삭제하려는 테이블과 연결된 FK 제약조건을 모두삭제 

DROP TABLE TB1 
CASCADE CONSTRAINTS; --테이블 삭제 시 FK 관계도 모두삭제

SELECT * FROM TB1;
SELECT * FROM TB2; --TB2 테이블만 아무관계 없이 남게됨 
...
-- 2) DDL 과 DML 구문 섞어서 수행하면 안된다. 
	--DDL(CREATE ALTER DROP  :객체 생성 수성 삭제)
	--DML(INSERT UPDATE DELETE)  : 데이텅 (행) 추가 / 갱신 / 삭제 

-->  DDL은 수행시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴 
--> DDL이 종료 된 후 DML 구문을 수행할수 있도록 권장 !
...
ROLLBACK;
SELECT* FROM TB2; --롤백 안된다. . 위에서 DDL 구문중  ALTER를 사용해서 그 시점에 COMMIT 됨 
