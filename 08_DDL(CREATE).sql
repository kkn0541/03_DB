 ...
 * *USER_CONSTRAINTS : 계정이 작성한 제약조건을 확인할 수 있는 딕셔너리뷰 
...
CREATE TABLE "MEMBER"(
	...
	MEMBER_SSN CHAR(14), --고정길이 CHAR 
	ENROLL_DATE DATE DEFAULT SYSDATE --현재시간을 가입일로 쓰겟다 
	
);
--만든 테이블 확인 
SELECT * FROM MEMBER;
 --2. 컬럼에 주석달기 
 --COMMENT ON COLUMN 테이블 IS '주석내용';
...
--1. NOT NULL 
-- 해당 컬러메 반드시 값이 기록되어야 하는 경우 사용 
--삽입 수정시 NULL 값이 허용하지 않도록 컬럼레벨에서 제한 

--* 컬럼레벨 : 테이블 생성 시 컬럼을 정의하는 부분에 작성하는 것 

CREATE TABLE USER_USED_NN(
	USER_NO NUMBER NOT NULL, --사용자 번호 (모든 사용자는 사용자 번호가 잇어야한다)
							--> 컬럼 레벨 제약조건 설정 
	USER_ID VARCHAR2(20),
...
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KKN"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
--컬럼레벨에서 설정 가능 , 테이블 레벨에서 설정가능 
--단, UNIQUE 제약조건이 설정된 컬럼에 NULL 값은 중복 삽입 가능 

--* 테이블 레벨 : 테이블 생성 시 컬럼 정의가 끝난 후 마지막에 작성 

--* 제약조건 지정방법 
--1) 컬럼 레벨   : [CONSTRAINT 제약조건명] 제약조건(UNIQUE , PRIMARY KEY....)
...
INSERT INTO USER_USED_UK 
VALUES (1,'USER01','PASS01','홍길동','남자','010-1234-5678','hong123@kh.or.kr');
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KKN.USER_ID_U)에 위배됩니다
--USER01이 이미 한번 들어갔기떄문에 중복허용되지 않는다. 
...
-- USER_ID 와 USER_NAME 중 ID는 다르기떄문에 가능 (둘중에 하나라도 다르면 가능)
...
INSERT INTO USER_USED_UK2 
VALUES (3,'USER01','PASS02','고길동','남자','010-1234-5678','hong123@kh.or.kr');
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KKN.USER_ID_NAME_U)에 위배됩니다
...
--테이블에서 한행의 정보를 찾기 위해 사용할 컬럼을 의미함 
--테이블에 대한 식별자 (사용자번호 , 학번 .. )역할을 함 

--NOT NULL + UNIQUE 제약조건의 의미 -> 중복되지 않는 값이 필수로 존재해야함

--한 테이블당 한개만 설정 할수 있음 
--컬럼 레벨, 테이블레벨 둘다 설정 가능 
-- 한 개의 컬럼에 설정할수 있고 , 여러개의 컬럼을 묶어서 설정할수 있음 (==PRIMARY 복합키)


CREATE  TABLE USER_USED_PK (
 	USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY,       --컬럼레벨(제약조건명 지정)중복 X NULL X 
	USER_ID VARCHAR2(20), 
	USER_PWD VARCHAR2(20), 
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10),
...
INSERT INTO USER_USED_PK 
VALUES (2,'USER03','PASS03','유관순','여자','010-1212-1212','YOO123@kh.or.kr');
--기본키 NULL 이므로 오류 ! 
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KKN"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_PK2 
VALUES (1,'USER02','PASS01','이순신','남자','010-1234-5678','LEE123@kh.or.kr');

INSERT INTO USER_USED_PK2 
VALUES (2,'USER01','PASS01','이순신','남자','010-1234-5678','LEE123@kh.or.kr');

INSERT INTO USER_USED_PK2 
VALUES (1,'USER01','PASS01','이순신','남자','010-1234-5678','lee123@kh.or.kr');
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KKN.PK_USERNO_USERID)에 위배됩니다
-- user_no user_id  둘다 중복되었을때만 제약조건 위배 에러 발생 

INSERT INTO USER_USED_PK2 
VALUES (NULL,'USER03','PASS01','이순신','남자','010-1234-5678','lee123@kh.or.kr');
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KKN"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

INSERT INTO USER_USED_PK2 
VALUES (3,NULL,'PASS01','이순신','남자','010-1234-5678','lee123@kh.or.kr');
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KKN"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다
...
VALUES (1,'USER01','PASS01','이순신','남자','010-1234-5678','lee123@kh.or.kr',10);
...
--SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH_KKN.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
--SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH_KKN.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
--30을 사용하고 있는 자식이 있기 때문에 삭제 안됨 

--GRADE_CODE 중 20은 외래키로 참조되고 있지 않으므로 삭제가 가능함 
DELETE FROM USER_GRADE WHERE GRADE_CODE =20;
SELECT * FROM USER_GRADE; --20삭제됨 
...
--부모테이블인 USER_GREADE2에서 GRADE_CODE= 10 삭제
...
--3) ON DELETE CASECASE : 부모키 삭제시 자식키도 함계 삭되
...
-- ON DELETE CASECADE  삭제 옵션이 적용된 자식 테이블 생성 
...
SELECT * FROM USER_GRADE3; --부모에서 10삭제
SELECT * FROM USER_USED_FK3 ; --부모의 10을 참조하고 있던 자식행 삭제됨 



-------------------------------------------------------

-- 5. CHECK 제약 조건  : 컬럼에 기록되는 값에 조건 설정을 할수 있음 
-- CHECK (컬럼명 비교연산자 비교값 )
...
CREATE TABLE USER_USED_CHECK(
	...
	USER_PWD VARCHAR2(20) NOT NULL, 
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10) CONSTRAINT GENDER CHECK(GENDER IN ('남','여')) ,
	PHONE VARCHAR2(30),
	EMAIL VARCHAR2(50)
	);

INSERT INTO USER_USED_CHECK
VALUES (1,'USER01','PASS01','안중근','남자','010-2222-1111','ahn123@kh.or.kr');
--SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_KKN.GENDER)이 위배되었습니다

INSERT INTO USER_USED_CHECK
VALUES (1,'USER01','PASS01','안중근','남','010-2222-1111','ahn123@kh.or.kr');
--가능
INSERT INTO USER_USED_CHECK
VALUES (2,'USER02','PASS02','유관순','여','010-2222-1111','ahn123@kh.or.kr');
--가능
INSERT INTO USER_USED_CHECK
VALUES (2,'USER02','PASS02','유관순','여자','010-2222-1111','ahn123@kh.or.kr');
--SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_KKN.GENDER)이 위배되었습니다
