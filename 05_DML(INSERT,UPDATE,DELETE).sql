...
SELECT * FROM EMPLOYEE2 WHERE EMP_NAME='유재식' OR EMP_NAME='방명수';
SELECT * FROM EMPLOYEE2 WHERE EMP_NAME IN('유재식','방명수');
...
SELECT * FROM EMP_M02; --5명 (기존 4명  + 신규1)
...
--롤백확인
SELECT * FROM EMPLOYEE2
WHERE EMP_NAME='홍길동'; --> 조회결과 있음 

--EMPLOYEE2테이블 전체 삭제 
DELETE FROM EMPLOYEE2; --24개행 삭제 
...
ROLLBACK;
--롤백 확인
SELECT * FROM EMPLOYEE3; --롤백 안됨 

--DELETE : 휴지통폴더에 버린것과 같음 ( 복구가능)
--TRUNCATE : SHIFT DEL  완전삭제 
