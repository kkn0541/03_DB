...
--단일행 함수 :N개의 값으 ㄹ읽어서 연산 후 N개의 결과를 반환  


--그룹함수 :N개의 값을 읽어서 연산 후 1개의 결과를 반환 (합ㄱㅖ, 평균, 최대,최소)
...
SELECT EMAIL,LENGTH(EMAIL)  --23개의 값을 받아서 23개의 값을 반환
FROM EMPLOYEE;
...
SELECT INSTR('AABAACAABBAA','B') FROM DUAL; --3 B의 위치가 3번째에있다. 

--문자열을 5번째 문자부터 검색하여 첫번째 B의 위치 조회 

SELECT INSTR ('AABAACAABBAA','B',5)FROM DUAL; --9 5번째 부터 5 6 7 8 9번째에있음
...
-- 칼럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내서 반환
-->잘라낼 길이 생략 시 끝까지 잘라냄 
--EMPLOYEE 테이블에서 사원명 이메일 중 아이디만 조회 
--sun_di@or.kr -> sun_di 

SELECT EMP_NAME,SUBSTR(EMAIL,1,INSTR(EMAIL,'@')-1)아이디 --골뱅이의 앞까지 자르겟다  
FROM EMPLOYEE;
...
SELECT '절대값 같음' FROM DUAL  -- 같으면 절대값 같음 을 출력하겠다 
WHERE ABS(10)= ABS(-10);
...
SELECT EMP_ID,EMP_NAME,MOD(EMP_ID,2)
FROM EMPLOYEE
...
--123.5  소수점 두번째 자리에서 반올림(소수점 첫번쨰 자리까지 표기)

SELECT ROUND(123.456,0) FROM DUAL; -- 소수점 첫번째 자리에서 반올림 

--CEIL(숫자 | 컬럼명) :올림
--FLOOR(숫자 | 컬럼명) :낼림
--> 둘다 소수점 첫째 자리에서 올림/낼림 처리 

SELECT CEIL (123.1), FLOOR(123.9) FROM DUAL;

--TRUNC(숫자 | 컬럼명[, 위치]) : 절삭 :특정 위치 아래를 절삭 

SELECT TRUNC(123.456) FROM DUAL; --123,소수점 아래를 절삭 
SELECT TRUNC(123.456,1) FROM DUAL; --123.4소수점첫째자리 아래 절삭
...
CEIL(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/12) || '년차'"근무년차"  --이어쓰기 ||
FROM EMPLOYEE;
...
SELECT LAST_DAY('2100-02-01') FROM DUAL; 
...
SELECT TO_CHAR(1234,'99999')FROM DUAL; --' 1234' 앞에 공백한칸있음 5칸마련해두고 
SELECT TO_CHAR(1234,'00000')FROM DUAL; --'01234' 
SELECT TO_CHAR(1234)FROM DUAL; --'01234' 
...
-->패턴을 적용해서 작성된 문자열의 각문자가 어떠난ㄹ짜 형식인지 인식 시킴 


--Y패턴 : 현재 세기(21세기 == 20XX년대)
--R패턴 : 1세기 기준으로 절반 (50년) 이상인 경우는 이전세기(1900년대)
--		 절반(50년) 미만인 경우 현재 세기(2000년대)

SELECT TO_DATE('800505','RRMMDD') FROM DUAL; -- 세기로(50년)크거나 미만 RR 1900년대로 나옴
SELECT TO_DATE('800505','YYMMDD') FROM DUAL; --현재세기  YY
SELECT TO_DATE('490505','RRMMDD') FROM DUAL; --현재세기  YY
...
--1)주민번호에서 - 앞글자까지 추출 
--2) 추출한 생년월일 DATE 타입으로 변경 ->RR 패턴을 이용하여 1900년도로 바꿈 
...
--3)TO_CHAR 를 이용해서 문자열로 변환 ->1965년 10월 08일 
...
--NVL(컬럼명,컬럼값이 NULL일때 바꿀값) : NULL인 컬럼값을 다른 값으로 변경 
...
--EMPLOYEE 테이블에서 보너를 받으면 'O' 안받으면 'X'조회
...
--비교하고자 하는 값 또는 컬럼이 조건식과 같으면 결과값반환 
...
-------AVG() 전직원 급여 평균 
SELECT ROUND(AVG(SALARY)) -- 3047663 반올림
FROM EMPLOYEE;

--부서코드가 'D9인 사원들의 급여 합,평균'
